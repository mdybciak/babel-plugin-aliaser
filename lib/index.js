...
function createFilesMap(file) {
  const result = {};
  const opts = Array.isArray(file.opts.extra.aliaser) ? file.opts.extra.aliaser : file.opts.extra.aliaser ? [file.opts.extra.aliaser] : [];
      ...
      ImportDeclaration: {
        enter(a, b, c, file) {
          ...
          if (modulePathToReplace) {
            return t.importDeclaration(a.specifiers, t.literal(modulePathToReplace));
          }
        }
      }
    } });
